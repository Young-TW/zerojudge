...
第一行有兩正整數 N（1 <= N <= 200,000）、M（1 <= M <= 200,000），如題幹所述，空地的編號為 1 ~ N。

接著M行，每行有三個正整數 a、b（1 <= a, b <= N）、w（1 <= w <= 1,000,000,000），代表空地 a 和空地 b 之間有條通道，通過這條通道的時間為 w。保證不存在自環和重邊。

最後一行有兩個正整數 S 和 T，S≠T，表示天使的初始位置在 S，基地在 T。
